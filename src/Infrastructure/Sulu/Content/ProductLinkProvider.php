 ...
 */
final class ProductLinkProvider implements LinkProviderInterface
{
    /**
     * @param class-string<ProductDimensionContentInterface> $productContentClass
     */
    public function __construct(
        ...
        private readonly string $productContentClass,
    ) {
    }
    ...
    public function getConfigurationBuilder(): LinkConfigurationBuilder
    {
        return LinkConfigurationBuilder::create()
            ->setTitle($this->translator->trans('sulu_product.products', [], 'admin'))
            ->setResourceKey(ProductInterface::RESOURCE_KEY)
            ->setListAdapter('table')
            ->setDisplayProperties(['title'])
            ->setOverlayTitle($this->translator->trans('sulu_product.selection_overlay_title', [], 'admin'))
            ->setEmptyText($this->translator->trans('sulu_product.no_product_selected', [], 'admin'))
            ->setIcon('su-document');
    }
    ...
    public function preload(array $hrefs, string $locale, bool $published = true): iterable
    {
        ...
        $rows = $this->findProductsByUuids($hrefs, $locale, $stage, $version, $requestWebspace);

        $result = [];
        foreach ($rows as $row) {
            $this->referenceStore->add($row['uuid'], ProductInterface::RESOURCE_KEY);
     ...
     */
    private function findProductsByUuids(
        array $uuids,
        string $locale,
        string $stage,
        int $version,
        ...
        $queryBuilder = $this->entityManager->createQueryBuilder()
            ->select('product.uuid', 'dimensionContent.title', 'route.slug', 'dimensionContent.mainWebspace')
            ->from($this->productContentClass, 'dimensionContent')
